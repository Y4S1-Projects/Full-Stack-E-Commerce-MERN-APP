...
const AdminPanel = () => {
  // Keep reading user from store for display, but don't block rendering
      ...
      <aside className="w-full min-h-full bg-white max-w-60 customShadow">
        <div className="flex flex-col items-center justify-center h-32">
          <div className="relative flex justify-center text-5xl cursor-pointer">
            {user?.profilePic ? (
              <img src={user?.profilePic} className="w-20 h-20 rounded-full" alt={user?.name || 'Admin'} />
            ) : (
              <FaRegCircleUser />
            )}
          </div>
          <p className="text-lg font-semibold capitalize">{user?.name || 'Guest'}</p>
